...
function parseBlockTag(cand) {
  ...
  return ethers.BigNumber.from(cand);
}
function eqBlockTag(a, b) {
  if (a === b) return true;
  if (ethers.BigNumber.isBigNumber(a)) {
    if (ethers.BigNumber.isBigNumber(b)) return a.eq(b);
    return false;
  }
  if (ethers.BigNumber.isBigNumber(b)) return false;
  return a === b;
}
...
class Multicall {
  constructor(options) {
    ...
    this.multicallInterface = new ethers.ethers.utils.Interface(abi.walletContracts.sequenceUtils.abi);
    this.options = void 0;
    this.timeout = void 0;
    this.queue = [];
    ...
    this.run = async function () {
      ...
      if (_this.options.verbose) console.log('Processing multicall');
      ...
      const reqId = utils.getRandomInt();

      // TODO: fix types below..

      const res = await safeSolve(
      // @ts-ignore
      utils.promisify(next)({
        id: reqId,
        jsonrpc: network.JsonRpcVersion,
        method: JsonRpcMethod.ethCall,
        params: [{
          to: _this.options.contract,
          value: 0,
          data: encodedCall
        }, ethers.BigNumber.isBigNumber(blockTag) ? blockTag.toNumber() : blockTag]
        // @ts-ignore
      }), e => ({
        jsonrpc: network.JsonRpcVersion,
        id: reqId,
          ...
          switch (item.request.method) {
            ...
            case JsonRpcMethod.ethGetCode:
              item.callback(undefined, {
                jsonrpc: item.request.jsonrpc,
                id: item.request.id,
                result: ethers.ethers.utils.defaultAbiCoder.decode(['bytes'], decoded[1][index])[0]
              });
              break;
            case JsonRpcMethod.ethGetBalance:
              item.callback(undefined, {
                jsonrpc: item.request.jsonrpc,
                id: item.request.id,
                result: ethers.ethers.utils.defaultAbiCoder.decode(['uint256'], decoded[1][index])[0]
              });
              break;
          }
        }
...
class MulticallProvider extends ethers.ethers.providers.BaseProvider {
  constructor(provider, multicall) {
    ...
    this.getResolver = async function (name) {
      ...
      if (provider.getResolver) {
        ...
        return new ethers.ethers.providers.Resolver(_this, ogResolver.address, ogResolver.name);
      }
  ...
  async rpcCall(method, ...params) {
    const reqId = utils.getRandomInt();
    const resp = await utils.promisify(this.multicall.handle)(this.next, {
      jsonrpc: network.JsonRpcVersion,
      id: reqId,
      method: method,
      params: params
