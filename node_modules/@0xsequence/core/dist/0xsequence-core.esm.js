...
function subdigestOf(payload) {
  return ethers.utils.solidityKeccak256(['bytes', 'uint256', 'address', 'bytes32'], ['0x1901', payload.chainId, payload.address, payload.digest]);
}
...
function encodeSignature$1(signature) {
  ...
  const encodedSigners = signers.map(s => {
    if (isAddressMember(s)) {
      return ethers.utils.solidityPack(['uint8', 'uint8', 'address'], [SignaturePartType$1.Address, s.weight, s.address]);
    }
    if (s.isDynamic) {
      const bytes = ethers.utils.arrayify(s.signature);
      return ethers.utils.solidityPack(['uint8', 'uint8', 'address', 'uint16', 'bytes'], [SignaturePartType$1.DynamicSignature, s.weight, s.address, bytes.length, bytes]);
    }
    return ethers.utils.solidityPack(['uint8', 'uint8', 'bytes'], [SignaturePartType$1.EOASignature, s.weight, s.signature]);
  });
...
const ConfigCoder$1 = {
  ...
  imageHashOf: config => {
    return config.signers.reduce((imageHash, signer) => ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'uint8', 'address'], [imageHash, signer.weight, signer.address])), ethers.utils.solidityPack(['uint256'], [config.threshold]));
  },
  ...
  buildStubSignature: function (config, overrides) {
    ...
    for (const {
      ...
      weight: b
    }) => ethers.BigNumber.from(a).sub(b).toNumber())) {
...
const partEncoder = {
  ...
  nested: (weight, threshold, tree) => {
    const arr = ethers.utils.arrayify(tree);
    return ethers.utils.solidityPack(['uint8', 'uint8', 'uint16', 'uint24', 'bytes'], [SignaturePartType.Nested, weight, threshold, arr.length, arr]);
  },
  subdigest: subdigest => {
  ...
  dynamicSignature: (weight, address, signature) => {
    ...
    return ethers.utils.solidityPack(['uint8', 'uint8', 'address', 'uint24', 'bytes'], [SignaturePartType.DynamicSignature, weight, address, arrSignature.length, arrSignature]);
  },
...
function encodeSigners(config, parts, subdigests, chainId, options = {}) {
  ...
  if (ethers.BigNumber.from(chainId).isZero()) {
    return {
      encoded: ethers.utils.solidityPack(['uint8', 'uint16', 'uint32', 'bytes'], [SignatureType.NoChainIdDynamic, config.threshold, config.checkpoint, tree.encoded]),
      weight: tree.weight
    };
  }
  if (ethers.BigNumber.from(config.threshold).gt(255)) {
    return {
      encoded: ethers.utils.solidityPack(['uint8', 'uint16', 'uint32', 'bytes'], [SignatureType.Dynamic, config.threshold, config.checkpoint, tree.encoded]),
      weight: tree.weight
    };
  }
  return {
    encoded: ethers.utils.solidityPack(['uint8', 'uint8', 'uint32', 'bytes'], [SignatureType.Legacy, config.threshold, config.checkpoint, tree.encoded]),
    weight: tree.weight
  };
}
...
function setImageHashStruct(imageHash) {
  return ethers.utils.solidityPack(['bytes32', 'bytes32'], [ethers.utils.solidityKeccak256(['string'], ['SetImageHash(bytes32 imageHash)']), imageHash]);
}
...
function encodeChain(main, suffix) {
  ...
  const body = ethers.utils.solidityPack(encodedMap.map(() => ['uint24', 'bytes']).flat(), encodedMap.map(s => [s.length, s]).flat());
...
function encodeSignatureBody(decoded) {
  return ethers.utils.solidityPack(['uint16', 'uint32', 'bytes'], [decoded.threshold, decoded.checkpoint, encodeSignatureTree(decoded.tree)]);
}
function encodeSignatureTree(tree) {
  if (isNode(tree) || isUnrecoveredNode(tree)) {
    ...
    if (isBranching) {
      return ethers.utils.solidityPack(['bytes', 'uint8', 'uint24', 'bytes'], [encodedLeft, SignaturePartType.Branch, encodedRight.length, encodedRight]);
    } else {
  ...
  if (isNestedLeaf(tree) || isUnrecoveredNestedLeaf(tree)) {
    ...
    return ethers.utils.solidityPack(['uint8', 'uint8', 'uint16', 'uint24', 'bytes'], [SignaturePartType.Nested, tree.weight, tree.threshold, nested.length, nested]);
  }
  if (isUnrecoveredSignatureLeaf(tree) || isSignerLeaf(tree) && tree.signature !== undefined) {
    ...
    if (tree.isDynamic || signature.length !== SignaturePartTypeLength) {
      ...
      return ethers.utils.solidityPack(['uint8', 'uint8', 'address', 'uint24', 'bytes'], [SignaturePartType.DynamicSignature, tree.weight, tree.address, signature.length, signature]);
    } else {
  ...
  if (isUnrecoveredNode(tree)) {
    ...
    if (left.weight === 0 && right.weight === 0) {
      try {
        ...
        const recovered = await recoverTopology(tree, ethers.constants.HashZero, undefined);
        return {
          weight: 0,
          trimmed: {
...
function hashNode(node) {
  ...
  if (isNestedLeaf(node)) {
    ...
    return ethers.utils.solidityKeccak256(['string', 'bytes32', 'uint256', 'uint256'], ['Sequence nested config:\n', nested, node.threshold, node.weight]);
  }
  if (isNodeLeaf(node)) {
...
function imageHash(config) {
  return ethers.utils.solidityKeccak256(['bytes32', 'uint256'], [ethers.utils.solidityKeccak256(['bytes32', 'uint256'], [hashNode(config.tree), config.threshold]), config.checkpoint]);
}
  ...
  while (stack.length > 0) {
    ...
    if (isNestedLeaf(node)) {
      ...
      signers.add({
        ...
        weight: ethers.BigNumber.from(node.weight).toNumber()
      });
    }
  }
...
const ConfigCoder = {
  ...
  update: {
    ...
    decodeTransaction: function (tx) {
      ...
      if (!ethers.constants.Zero.eq((_tx$transactions$0$va = (_tx$transactions$ = tx.transactions[0]) == null ? void 0 : _tx$transactions$.value) != null ? _tx$transactions$0$va : 0)) {
        throw new Error('Invalid transaction bundle, expected value to be 0');
      }
      if (!ethers.constants.Zero.eq((_tx$transactions$0$ga = (_tx$transactions$2 = tx.transactions[0]) == null ? void 0 : _tx$transactions$2.gasLimit) != null ? _tx$transactions$0$ga : 0)) {
        throw new Error('Invalid transaction bundle, expected value to be 0');
      }
      return {
...
function addressOf(context, imageHash) {
  const codeHash = ethers.utils.keccak256(ethers.utils.solidityPack(['bytes', 'bytes32'], [context.walletCreationCode, ethers.utils.hexZeroPad(context.mainModule, 32)]));
  const hash = ethers.utils.keccak256(ethers.utils.solidityPack(['bytes1', 'address', 'bytes32', 'bytes32'], ['0xff', context.factory, imageHash, codeHash]));
...
function intendedTransactionID(bundle) {
  return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'bytes32'], [bundle.intent.wallet, bundle.chainId, bundle.intent.id]));
}
...
function subdigestOfGuestModuleTransactions(guestModule, chainId, txs) {
  return subdigestOf({
    address: guestModule,
    chainId,
    digest: ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['string', MetaTransactionsType], ['guest:', sequenceTxAbiEncode(txs)]))
  });
}
...
function encodeBundleExecData(bundle) {
  ...
  sequenceTxAbiEncode(bundle.transactions), 0, []]);
}
...
async function validateEIP6492Offchain(provider, signer, hash, signature) {
  return '0x01' === (await provider.call({
    data: ethers.utils.concat([EIP_6492_OFFCHAIN_DEPLOY_CODE, new ethers.utils.AbiCoder().encode(['address', 'bytes32', 'bytes'], [signer, hash, signature])])
  }));
}
...
class OnChainReader {
  ...
  module(address) {
    return new ethers.Contract(address, [...walletContracts.mainModuleUpgradable.abi, ...walletContracts.mainModule.abi, ...walletContracts.erc1271.abi], this.provider);
  }
...
export { VERSION, allVersions, index$1 as commons, index as universal, v1, v2 };
